import type { MethodFilter } from '../types'
import { METHODS } from '../lib/tags'

/** 做法筛选 chips 的 props */
interface MethodChipsProps {
  active: MethodFilter
  onChange: (method: MethodFilter) => void
}

/** 做法筛选选项：全部 + METHODS 枚举（值即中文标签） */
const METHOD_CHIPS: MethodFilter[] = ['all', ...METHODS]

/**
 * 做法筛选 chips：与分类 / 餐次 / 菜系 / 口味筛选组合过滤
 * 圆角胶囊样式与 MealTypeChips 一致（无数量角标），超出换行
 */
export default function MethodChips({ active, onChange }: MethodChipsProps) {
  return (
    <div className="flex flex-wrap items-center gap-2">
      {METHOD_CHIPS.map((method) => {
        const isActive = active === method
        return (
          <button
            key={method}
            onClick={() => onChange(method)}
            aria-pressed={isActive}
            className={`rounded-full px-3 py-1 text-xs font-semibold transition-all duration-300 active:scale-95 ${
              isActive
                ? 'scale-105 bg-gradient-brand text-white shadow-md shadow-grape/20'
                : 'border border-white/10 bg-white/5 text-mist hover:scale-[1.03] hover:border-grape/40 hover:text-snow'
            }`}
          >
            {method === 'all' ? '全部' : method}
          </button>
        )
      })}
    </div>
  )
}
