 ...
 * 可选 onOpen(meal)：点击某道菜条目打开做法详情弹窗（不传则条目不可点击，保持原行为）。
 */
...
// 从候选列表里随机取一道（不改变传入数组）
function pickOne(candidates, rand) {
 ...
 */
function pickFromPool(poolKey, meals, cuisineFilter, rand, usedIds) {
...
function generateTable(meals, cuisineFilter, seed) {
...
function replaceDish(table, dishId, poolKey, meals, cuisineFilter) {
...
export default function NewYearDinner({ meals, cuisineFilter, onAddAll, onOpen }) {
