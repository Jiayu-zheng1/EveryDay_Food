 ...
 */
export default function Hero({ mealCount, onNavigate }) {
  const primaryRef = useRef(null)
  const titleRef = useRef(null) // h1 标题，内含 kinetic 词块
  const spotRefs = useRef([]) // 视差光斑（外层包裹 div，动画在子元素上）

  // 鼠标在按钮上移动时更新 --gx/--gy，驱动光晕跟随
  const handlePrimaryMove = (e) => {
    ...
    const onScroll = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        const words = titleRef.current
          ? Array.from(titleRef.current.querySelectorAll('.kinetic-word'))
          : []
