...
type NumericFieldKey = 'age' | 'heightCm' | 'weightKg'
...
// 宏量营养条配置（与弹窗一致的展示风格）
const MACRO_BARS: Array<{ key: 'proteinG' | 'carbsG' | 'fatG'; label: string; bar: string }> = [
...
const KCAL_PER_GRAM: Record<'proteinG' | 'carbsG' | 'fatG', number> = { proteinG: 4, carbsG: 4, fatG: 9 }
...
export default function CalorieCalculator({
  ...
  // 真正的数字归一（空串按 0）统一在计算函数入口做（见 lib/nutrition.ts 的 toNumber）。
  const updateNumber =
    (key: NumericFieldKey) => (e: ChangeEvent<HTMLInputElement>) =>
