...
export default function App() {
  ...
  const [mealTypeFilter, setMealTypeFilter] = useState('all') // 菜谱库餐次筛选
  const [cuisineFilter, setCuisineFilter] = useState('all') // 菜谱库地区筛选（年夜饭生成也按此优先）
  ...
  const sentinelRef = useRef(null) // 网格末尾哨兵：进入视口时加载更多
  const cursorLightRef = useRef(null) // 全屏鼠标跟随光源层
  ...
  const effectivePlan = nutrition.smartPlan ?? plan
  ...
  const counts = useMemo(() => {
    const result = { all: meals.length }
    for (const key of Object.keys(categoryMeta)) {
      result[key] = meals.filter((meal) => meal.category === key).length
    }
    return result
  }, [meals, categoryMeta])
  ...
  const cuisineCounts = useMemo(() => {
    ...
    for (const key of Object.keys(CUISINE_META)) {
      if (key !== 'all') result[key] = meals.filter((m) => m.cuisine === key).length
    }
    return result
  }, [meals])
