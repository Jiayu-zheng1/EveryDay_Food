...
/**
 * 解析单个「名称 + 数量 + 单位」片段
 * @returns {{ seasoning: true, name: string } | { name: string, amount: number, unit: string } | null}
 */
function parseQtyPart(part) {
  const s = part.trim()
  if (!s) return null
 ...
 * @returns {Array<{ name: string, amount: number, unit: string }> | null}
 */
...
/**
 * 把一组菜品的食材清单合并成购物清单
 * @param {Array<{ ingredients?: string[] }>} items 已选菜品
 * @returns {{ rows: Array, groups: string[] }}
 */
export function buildShoppingList(items) {
  /** key: `${group}|${name}|${unit}` → 已合并的数量行 */
  const qtyRows = new Map()
  /** 调味品按名称去重（只列名称不合并数量） */
  const seasoningSeen = new Set()
  const rows = []

  // 数量行：同名称 + 同单位 → 数量相加
  const emitQty = (name, amount, unit, raw) => {
    ...
    const row = { name, amount: roundAmount(amount), unit, raw, group: '食材' }
    qtyRows.set(key, row)
    rows.push(row)
  }

  // 调味品行：只列名称，同名去重
  const emitSeasoning = (name, raw) => {
  ...
  for (const meal of items ?? []) {
    for (const rawIng of meal.ingredients ?? []) {
      ...
      const parsed = parts.map(parseQtyPart)
